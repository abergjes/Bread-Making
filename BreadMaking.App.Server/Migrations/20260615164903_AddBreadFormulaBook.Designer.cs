// <auto-generated />
using System;
using BreadMaking.App.Server.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace BreadMaking.App.Server.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260615164903_AddBreadFormulaBook")]
    partial class AddBreadFormulaBook
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "9.0.5");

            modelBuilder.Entity("BreadMaking.App.Server.Data.Bake", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<double?>("AmbientHumidityPct")
                        .HasColumnType("REAL");

                    b.Property<double?>("AmbientTempC")
                        .HasColumnType("REAL");

                    b.Property<double?>("ButterPct")
                        .HasColumnType("REAL");

                    b.Property<double?>("EggPct")
                        .HasColumnType("REAL");

                    b.Property<DateTimeOffset?>("EndedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("FlourBatch")
                        .HasColumnType("TEXT");

                    b.Property<double?>("HydrationPct")
                        .HasColumnType("REAL");

                    b.Property<double?>("InoculationPct")
                        .HasColumnType("REAL");

                    b.Property<bool>("IsPullmanTin")
                        .HasColumnType("INTEGER");

                    b.Property<double?>("MilkPct")
                        .HasColumnType("REAL");

                    b.Property<double?>("MilkPowderPct")
                        .HasColumnType("REAL");

                    b.Property<string>("Notes")
                        .HasColumnType("TEXT");

                    b.Property<int>("RecipeId")
                        .HasColumnType("INTEGER");

                    b.Property<double?>("SaltPct")
                        .HasColumnType("REAL");

                    b.Property<DateTimeOffset>("StartedAt")
                        .HasColumnType("TEXT");

                    b.Property<int?>("StarterActivity")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("StarterFeedLogId")
                        .HasColumnType("INTEGER");

                    b.Property<double?>("SugarPct")
                        .HasColumnType("REAL");

                    b.Property<double?>("TotalFlourGrams")
                        .HasColumnType("REAL");

                    b.HasKey("Id");

                    b.HasIndex("RecipeId");

                    b.HasIndex("StarterFeedLogId");

                    b.ToTable("Bakes");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.BakeOutcome", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("BakeId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("CrumbNotes")
                        .HasColumnType("TEXT");

                    b.Property<int?>("CrumbOpenness")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("CrustScore")
                        .HasColumnType("INTEGER");

                    b.Property<double?>("InternalTempC")
                        .HasColumnType("REAL");

                    b.Property<bool>("IsBestLoaf")
                        .HasColumnType("INTEGER");

                    b.Property<double?>("LoafHeightCm")
                        .HasColumnType("REAL");

                    b.Property<double?>("OvenSpringPct")
                        .HasColumnType("REAL");

                    b.Property<int?>("OverallScore")
                        .HasColumnType("INTEGER");

                    b.Property<string>("PhotoPath")
                        .HasColumnType("TEXT");

                    b.Property<int?>("ProofingResult")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Tags")
                        .HasColumnType("TEXT");

                    b.Property<int?>("TasteScore")
                        .HasColumnType("INTEGER");

                    b.Property<double?>("WeightLossPct")
                        .HasColumnType("REAL");

                    b.HasKey("Id");

                    b.HasIndex("BakeId")
                        .IsUnique();

                    b.ToTable("BakeOutcomes");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.BakeStepLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("BakeId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTimeOffset?>("EndedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("Notes")
                        .HasColumnType("TEXT");

                    b.Property<int>("PlannedDurationMin")
                        .HasColumnType("INTEGER");

                    b.Property<int>("RecipeStepId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTimeOffset?>("StartedAt")
                        .HasColumnType("TEXT");

                    b.Property<int>("Status")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("BakeId");

                    b.HasIndex("RecipeStepId");

                    b.ToTable("BakeStepLogs");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.BreadFormula", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("Notes")
                        .HasColumnType("TEXT");

                    b.Property<string>("Tags")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("BreadFormulas");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.FormulaIngredient", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("FormulaId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<decimal>("Percent")
                        .HasColumnType("TEXT");

                    b.Property<int>("SortOrder")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("FormulaId");

                    b.ToTable("FormulaIngredients");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.GrainProfileEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("FlavorNotes")
                        .HasColumnType("TEXT");

                    b.Property<string>("GlutenStrength")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("HistoricalOrigin")
                        .HasColumnType("TEXT");

                    b.Property<double>("HydrationAdjustPct")
                        .HasColumnType("REAL");

                    b.Property<int>("MaxAutolyseMinutes")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<bool>("NeedsBinder")
                        .HasColumnType("INTEGER");

                    b.Property<string>("NutritionHighlights")
                        .HasColumnType("TEXT");

                    b.Property<string>("Ploidy")
                        .HasColumnType("TEXT");

                    b.Property<string>("UsageNotes")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("GrainProfiles", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            FlavorNotes = "Mild, slightly sweet; neutral backdrop that lets fermentation flavours shine",
                            GlutenStrength = "Strong",
                            HistoricalOrigin = "Bred since 19th century from emmer × einkorn crosses; global staple",
                            HydrationAdjustPct = 0.0,
                            MaxAutolyseMinutes = 60,
                            Name = "Modern wheat",
                            NeedsBinder = false,
                            NutritionHighlights = "High gluten (13–14%); refined versions stripped of bran and germ",
                            Ploidy = "Hexaploid",
                            UsageNotes = "Universal base flour; ideal 100% for most sourdough and sandwich loaves"
                        },
                        new
                        {
                            Id = 2,
                            FlavorNotes = "Earthy, nutty, slightly bitter from bran; deeper character than white wheat",
                            GlutenStrength = "Strong",
                            HistoricalOrigin = "Same hexaploid wheat as white flour; nothing removed in milling",
                            HydrationAdjustPct = 5.0,
                            MaxAutolyseMinutes = 60,
                            Name = "Whole grain",
                            NeedsBinder = false,
                            NutritionHighlights = "Retains bran and germ; high fibre, B vitamins, iron, zinc",
                            Ploidy = "Hexaploid",
                            UsageNotes = "25–50% blend for nutrition boost; absorbs 5% more water than white"
                        },
                        new
                        {
                            Id = 3,
                            FlavorNotes = "Sour, dark, slightly earthy; pronounced complexity with long fermentation",
                            GlutenStrength = "Very low (bran)",
                            HistoricalOrigin = "Secale cereale; cultivated in Central Europe since ~2000 BC; Nordic staple",
                            HydrationAdjustPct = 15.0,
                            MaxAutolyseMinutes = 60,
                            Name = "Rye",
                            NeedsBinder = false,
                            NutritionHighlights = "High pentosan (soluble fibre); low GI; high lysine; rich in B vitamins",
                            UsageNotes = "10–30% blend for flavour depth; high percentage gives dense Nordic-style crumb"
                        },
                        new
                        {
                            Id = 4,
                            FlavorNotes = "Mildly nutty, slightly sweet; lighter character than whole wheat",
                            GlutenStrength = "Moderate (fragile)",
                            HistoricalOrigin = "Triticum spelta; hexaploid ancient grain cultivated in Europe since ~5000 BC",
                            HydrationAdjustPct = -5.0,
                            MaxAutolyseMinutes = 30,
                            Name = "Spelt",
                            NeedsBinder = false,
                            NutritionHighlights = "Higher protein than modern wheat; soluble form; better B vitamin profile",
                            Ploidy = "Hexaploid",
                            UsageNotes = "Sub 1:1 for wheat but handle gently; gluten is extensible, not elastic"
                        },
                        new
                        {
                            Id = 5,
                            FlavorNotes = "Rich, buttery, almost corn-like; complex malty sweetness",
                            GlutenStrength = "Very weak",
                            HistoricalOrigin = "First cultivated wheat; diploid; Fertile Crescent ~10 000 BC",
                            HydrationAdjustPct = -15.0,
                            MaxAutolyseMinutes = 15,
                            Name = "Einkorn",
                            NeedsBinder = false,
                            NutritionHighlights = "High carotenoids (golden crumb); good zinc and B6; less starch than wheat",
                            Ploidy = "Diploid",
                            UsageNotes = "15–30% max in blends; very weak gluten — short autolyse, folds only"
                        },
                        new
                        {
                            Id = 6,
                            FlavorNotes = "Earthy, nutty, rustic; hint of bitterness from bran",
                            GlutenStrength = "Weak",
                            HistoricalOrigin = "Tetraploid; domesticated in Fertile Crescent ~8000 BC; key ancient grain",
                            HydrationAdjustPct = -10.0,
                            MaxAutolyseMinutes = 20,
                            Name = "Emmer (farro)",
                            NeedsBinder = false,
                            NutritionHighlights = "High fibre, iron, magnesium; richer protein than modern wheat",
                            Ploidy = "Tetraploid",
                            UsageNotes = "30–50% in sourdough blends; contributes open crumb at lower ratios"
                        },
                        new
                        {
                            Id = 7,
                            FlavorNotes = "Sweet, buttery, slightly rich; milder sour note than modern wheat sourdough",
                            GlutenStrength = "Strong",
                            HistoricalOrigin = "Khorasan wheat; tetraploid; ancient Egyptian grain; trademarked as KAMUT®",
                            HydrationAdjustPct = 10.0,
                            MaxAutolyseMinutes = 45,
                            Name = "Kamut (khorasan)",
                            NeedsBinder = false,
                            NutritionHighlights = "25% more protein than modern wheat; high selenium and zinc",
                            Ploidy = "Tetraploid",
                            UsageNotes = "50–100% sourdough loaves; benefits from longer autolyse (30–45 min)"
                        },
                        new
                        {
                            Id = 8,
                            FlavorNotes = "Earthy, molasses-like, slightly chocolatey; strong and distinctive",
                            GlutenStrength = "None (GF)",
                            HistoricalOrigin = "Grass grain from Ethiopia; staple for millennia; world's smallest cereal grain",
                            HydrationAdjustPct = 0.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Teff",
                            NeedsBinder = true,
                            NutritionHighlights = "Gluten-free; very high iron and calcium; good fibre; complete amino acids",
                            UsageNotes = "10–20% in GF blends with binder; backbone of Ethiopian injera at 100%"
                        },
                        new
                        {
                            Id = 9,
                            FlavorNotes = "Mild, slightly sweet, neutral; versatile GF base flour",
                            GlutenStrength = "None (GF)",
                            HistoricalOrigin = "Grass grain originating in Africa ~8000 years ago; 5th most grown cereal worldwide",
                            HydrationAdjustPct = 0.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Sorghum",
                            NeedsBinder = true,
                            NutritionHighlights = "Gluten-free; high protein; good antioxidants; cholesterol-lowering compounds",
                            UsageNotes = "Base GF flour 50–60%; combine with tapioca starch and psyllium husk binder"
                        },
                        new
                        {
                            Id = 10,
                            FlavorNotes = "Malty, slightly sweet; classic fermentation base note",
                            GlutenStrength = "Very low",
                            HistoricalOrigin = "One of the first cultivated grains; Fertile Crescent ~10 000 BC; foundation of beer",
                            HydrationAdjustPct = 0.0,
                            MaxAutolyseMinutes = 30,
                            Name = "Barley",
                            NeedsBinder = false,
                            NutritionHighlights = "Very high in soluble beta-glucan fibre; low GI; good selenium",
                            Ploidy = "Diploid",
                            UsageNotes = "10–30% blend for malty depth; high beta-glucan makes dough sticky"
                        },
                        new
                        {
                            Id = 11,
                            FlavorNotes = "Rich, golden, slightly sweet and nutty; dense satisfying crumb",
                            GlutenStrength = "Strong (stiff)",
                            HistoricalOrigin = "Tetraploid wheat; Mediterranean staple; primarily milled for pasta and couscous",
                            HydrationAdjustPct = 5.0,
                            MaxAutolyseMinutes = 30,
                            Name = "Durum / Semolina",
                            NeedsBinder = false,
                            NutritionHighlights = "High protein; rich in carotenoids giving yellow colour; good iron",
                            Ploidy = "Tetraploid",
                            UsageNotes = "10–40% for golden crumb and richness; classic in pane di Altamura (100%)"
                        },
                        new
                        {
                            Id = 12,
                            FlavorNotes = "Mild rye-wheat hybrid; slightly earthy with a hint of sweetness",
                            GlutenStrength = "Moderate",
                            HistoricalOrigin = "First man-made grain; rye x wheat hybrid developed in Scotland 1875",
                            HydrationAdjustPct = 5.0,
                            MaxAutolyseMinutes = 40,
                            Name = "Triticale",
                            NeedsBinder = false,
                            NutritionHighlights = "Higher protein and lysine than wheat; good fibre; bred for nutrition",
                            Ploidy = "Hexaploid",
                            UsageNotes = "20–50% blend; behaves like mild rye; excellent starter food"
                        },
                        new
                        {
                            Id = 13,
                            FlavorNotes = "Creamy, mild, slightly toasty; adds sweetness and a chewy crust",
                            GlutenStrength = "None (GF)",
                            HistoricalOrigin = "Avena sativa; cultivated in Europe since ~3000 years ago; Scottish staple grain",
                            HydrationAdjustPct = 10.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Oat",
                            NeedsBinder = true,
                            NutritionHighlights = "Naturally gluten-free (check cross-contamination); high soluble beta-glucan; heart-healthy",
                            Ploidy = "Hexaploid",
                            UsageNotes = "10–20% rolled or flour; use with psyllium binder in GF loaves"
                        },
                        new
                        {
                            Id = 14,
                            FlavorNotes = "Bold, earthy, slightly bitter; pronounced and distinctive flavour",
                            GlutenStrength = "None (GF)",
                            HistoricalOrigin = "Not a true cereal; Fagopyrum genus; Central Asia; cultivated ~8000 years ago",
                            HydrationAdjustPct = 5.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Buckwheat",
                            NeedsBinder = true,
                            NutritionHighlights = "Gluten-free; complete protein (all 9 amino acids); high rutin antioxidant",
                            UsageNotes = "10–20% in blends for strong flavour; classic in blini and French galettes"
                        },
                        new
                        {
                            Id = 15,
                            FlavorNotes = "Earthy, slightly grassy and peppery; intense flavour dominates blends",
                            GlutenStrength = "None (GF)",
                            HistoricalOrigin = "Pseudocereal of the Americas; Aztec sacred staple ~6000 years ago",
                            HydrationAdjustPct = 5.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Amaranth",
                            NeedsBinder = true,
                            NutritionHighlights = "Gluten-free; high quality protein (high lysine); rich in iron and calcium",
                            UsageNotes = "5–10% maximum — bold flavour; best in multigrain or seeded loaves"
                        },
                        new
                        {
                            Id = 16,
                            FlavorNotes = "Mild, slightly nutty and earthy; bitter if seeds not rinsed (saponins)",
                            GlutenStrength = "None (GF)",
                            HistoricalOrigin = "Pseudocereal from Andean South America; Inca sacred crop; ~7000 years ago",
                            HydrationAdjustPct = 5.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Quinoa",
                            NeedsBinder = true,
                            NutritionHighlights = "Gluten-free; complete protein; high magnesium, folate, and phosphorus",
                            UsageNotes = "5–15% as flour; rinse seeds before milling; toast for richer flavour"
                        },
                        new
                        {
                            Id = 17,
                            FlavorNotes = "Mild, slightly sweet and corn-like; light and delicate character",
                            GlutenStrength = "None (GF)",
                            HistoricalOrigin = "Panicum miliaceum; one of the first domesticated grains; Asia and Africa ~10 000 BC",
                            HydrationAdjustPct = 0.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Millet",
                            NeedsBinder = true,
                            NutritionHighlights = "Gluten-free; good iron and B vitamins; alkaline-forming grain",
                            UsageNotes = "10–20% in GF blends for lightness; contributes golden colour to crumb"
                        },
                        new
                        {
                            Id = 18,
                            FlavorNotes = "Neutral, soft and pillowy; designed to carry fillings and steam-imparted sweetness",
                            GlutenStrength = "Weak (cake/pastry)",
                            HistoricalOrigin = "Low-protein milling fractions; used throughout East Asia for millennia in steamed breads",
                            HydrationAdjustPct = -15.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Low-protein wheat",
                            NeedsBinder = false,
                            NutritionHighlights = "9–11% protein; low gluten gives tender, fine crumb; lower in fibre than whole grain",
                            Ploidy = "Hexaploid",
                            UsageNotes = "Use cake or pastry flour for Mantou / Baozi; not suitable for crust-forming bakes"
                        },
                        new
                        {
                            Id = 19,
                            FlavorNotes = "Rich, buttery, subtly sweet; crumb is cloud-soft with a gentle pull",
                            GlutenStrength = "Strong",
                            HistoricalOrigin = "Shokupan (食パン) perfected in Japan in the early 20th century; Tangzhong method popularised by Yvonne Chen (2007)",
                            HydrationAdjustPct = 0.0,
                            MaxAutolyseMinutes = 0,
                            Name = "Enriched",
                            NeedsBinder = false,
                            NutritionHighlights = "Higher in fat and sugar than lean bread; roux pre-gelatinises starch for moisture retention",
                            Ploidy = "Hexaploid",
                            UsageNotes = "Use high-protein bread flour (12–14%) for structure to hold the enrichment; develop gluten before adding fat"
                        });
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Measurement", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("BakeStepLogId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("MeasurementTypeId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTimeOffset>("RecordedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("Unit")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<double>("Value")
                        .HasColumnType("REAL");

                    b.HasKey("Id");

                    b.HasIndex("BakeStepLogId");

                    b.HasIndex("MeasurementTypeId");

                    b.ToTable("Measurements");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.MeasurementType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("DefaultForPhase")
                        .HasColumnType("TEXT");

                    b.Property<double?>("MaxValid")
                        .HasColumnType("REAL");

                    b.Property<double?>("MinValid")
                        .HasColumnType("REAL");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("Unit")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("MeasurementTypes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Category = "InProcess",
                            DefaultForPhase = "Mix",
                            MaxValid = 40.0,
                            MinValid = 10.0,
                            Name = "Dough temp",
                            Unit = "°C"
                        },
                        new
                        {
                            Id = 2,
                            Category = "InProcess",
                            DefaultForPhase = "Bulk",
                            MaxValid = 200.0,
                            MinValid = 0.0,
                            Name = "Aliquot rise",
                            Unit = "%"
                        },
                        new
                        {
                            Id = 3,
                            Category = "InProcess",
                            DefaultForPhase = "Bulk",
                            MaxValid = 7.0,
                            MinValid = 3.0,
                            Name = "pH",
                            Unit = "pH"
                        },
                        new
                        {
                            Id = 4,
                            Category = "InProcess",
                            DefaultForPhase = "Bulk",
                            MaxValid = 30.0,
                            MinValid = 0.0,
                            Name = "TTA",
                            Unit = "mL"
                        },
                        new
                        {
                            Id = 5,
                            Category = "Outcome",
                            DefaultForPhase = "Bake",
                            MaxValid = 110.0,
                            MinValid = 80.0,
                            Name = "Internal temp",
                            Unit = "°C"
                        });
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Recipe", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("CreatedByLabel")
                        .HasColumnType("TEXT");

                    b.Property<double>("FrictionFactorC")
                        .HasColumnType("REAL");

                    b.Property<int?>("GrainProfileId")
                        .HasColumnType("INTEGER");

                    b.Property<bool>("IsUserDefined")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Method")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<double>("TargetDoughTempC")
                        .HasColumnType("REAL");

                    b.Property<double>("TargetHydrationPct")
                        .HasColumnType("REAL");

                    b.HasKey("Id");

                    b.HasIndex("GrainProfileId");

                    b.ToTable("Recipes");

                    b.HasData(
                        new
                        {
                            Id = 13,
                            FrictionFactorC = 0.0,
                            GrainProfileId = 18,
                            IsUserDefined = false,
                            Method = 3,
                            Name = "Low-protein wheat — Steamed",
                            TargetDoughTempC = 26.0,
                            TargetHydrationPct = 57.0
                        },
                        new
                        {
                            Id = 14,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 19,
                            IsUserDefined = false,
                            Method = 4,
                            Name = "Enriched — Shokupan (Tangzhong)",
                            TargetDoughTempC = 26.0,
                            TargetHydrationPct = 65.0
                        },
                        new
                        {
                            Id = 15,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 1,
                            IsUserDefined = false,
                            Method = 5,
                            Name = "Modern wheat — Commercial yeast",
                            TargetDoughTempC = 25.0,
                            TargetHydrationPct = 72.0
                        },
                        new
                        {
                            Id = 1,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 1,
                            IsUserDefined = false,
                            Method = 0,
                            Name = "Modern wheat — Autolyse",
                            TargetDoughTempC = 25.0,
                            TargetHydrationPct = 72.0
                        },
                        new
                        {
                            Id = 2,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 1,
                            IsUserDefined = false,
                            Method = 1,
                            Name = "Modern wheat — Fermentolyse",
                            TargetDoughTempC = 25.0,
                            TargetHydrationPct = 72.0
                        },
                        new
                        {
                            Id = 3,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 4,
                            IsUserDefined = false,
                            Method = 0,
                            Name = "Spelt — Autolyse",
                            TargetDoughTempC = 24.0,
                            TargetHydrationPct = 68.0
                        },
                        new
                        {
                            Id = 4,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 4,
                            IsUserDefined = false,
                            Method = 1,
                            Name = "Spelt — Fermentolyse",
                            TargetDoughTempC = 24.0,
                            TargetHydrationPct = 68.0
                        },
                        new
                        {
                            Id = 5,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 5,
                            IsUserDefined = false,
                            Method = 0,
                            Name = "Einkorn — Autolyse",
                            TargetDoughTempC = 24.0,
                            TargetHydrationPct = 62.0
                        },
                        new
                        {
                            Id = 6,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 5,
                            IsUserDefined = false,
                            Method = 1,
                            Name = "Einkorn — Fermentolyse",
                            TargetDoughTempC = 24.0,
                            TargetHydrationPct = 62.0
                        },
                        new
                        {
                            Id = 7,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 6,
                            IsUserDefined = false,
                            Method = 0,
                            Name = "Emmer — Autolyse",
                            TargetDoughTempC = 24.0,
                            TargetHydrationPct = 65.0
                        },
                        new
                        {
                            Id = 8,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 6,
                            IsUserDefined = false,
                            Method = 1,
                            Name = "Emmer — Fermentolyse",
                            TargetDoughTempC = 24.0,
                            TargetHydrationPct = 65.0
                        },
                        new
                        {
                            Id = 9,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 7,
                            IsUserDefined = false,
                            Method = 0,
                            Name = "Kamut — Autolyse",
                            TargetDoughTempC = 25.0,
                            TargetHydrationPct = 78.0
                        },
                        new
                        {
                            Id = 10,
                            FrictionFactorC = 4.0,
                            GrainProfileId = 7,
                            IsUserDefined = false,
                            Method = 1,
                            Name = "Kamut — Fermentolyse",
                            TargetDoughTempC = 25.0,
                            TargetHydrationPct = 78.0
                        },
                        new
                        {
                            Id = 11,
                            FrictionFactorC = 0.0,
                            GrainProfileId = 8,
                            IsUserDefined = false,
                            Method = 2,
                            Name = "Teff — Soaker",
                            TargetDoughTempC = 24.0,
                            TargetHydrationPct = 95.0
                        },
                        new
                        {
                            Id = 12,
                            FrictionFactorC = 0.0,
                            GrainProfileId = 9,
                            IsUserDefined = false,
                            Method = 2,
                            Name = "Sorghum — Soaker",
                            TargetDoughTempC = 24.0,
                            TargetHydrationPct = 90.0
                        });
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.RecipeFormula", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<double>("FlourWeightG")
                        .HasColumnType("REAL");

                    b.Property<string>("Notes")
                        .HasColumnType("TEXT");

                    b.Property<int>("RecipeId")
                        .HasColumnType("INTEGER");

                    b.Property<double>("SaltPct")
                        .HasColumnType("REAL");

                    b.Property<double>("StarterPct")
                        .HasColumnType("REAL");

                    b.Property<double>("WaterPct")
                        .HasColumnType("REAL");

                    b.HasKey("Id");

                    b.HasIndex("RecipeId")
                        .IsUnique();

                    b.ToTable("RecipeFormulas");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.RecipeStep", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("DefaultDurationMin")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Description")
                        .HasColumnType("TEXT");

                    b.Property<int>("MaxDurationMin")
                        .HasColumnType("INTEGER");

                    b.Property<int>("MinDurationMin")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("Order")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Phase")
                        .HasColumnType("TEXT");

                    b.Property<int>("RecipeId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("StepMin")
                        .HasColumnType("INTEGER");

                    b.Property<double?>("TargetTempC")
                        .HasColumnType("REAL");

                    b.HasKey("Id");

                    b.HasIndex("RecipeId");

                    b.ToTable("RecipeSteps");

                    b.HasData(
                        new
                        {
                            Id = 101,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour and water only — no salt, no starter yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 1,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 102,
                            DefaultDurationMin = 50,
                            Description = "Cover and leave undisturbed. Enzymes hydrate flour and begin softening gluten.",
                            MaxDurationMin = 60,
                            MinDurationMin = 40,
                            Name = "Autolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 1,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 103,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate. Add starter and fold in fully.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt + starter",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 1,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 104,
                            DefaultDurationMin = 300,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 360,
                            MinDurationMin = 240,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 1,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 105,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 1,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 106,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 1,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 107,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 1,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 108,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 1,
                            StepMin = 30,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 109,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 1,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 110,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 1,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 111,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 1,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 112,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 1,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 201,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour, water, and starter — no salt yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water + starter",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 2,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 202,
                            DefaultDurationMin = 75,
                            Description = "Cover and leave undisturbed. Fermentation begins — watch for over-activity in warm kitchens.",
                            MaxDurationMin = 90,
                            MinDurationMin = 60,
                            Name = "Fermentolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 2,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 203,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate thoroughly.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 2,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 204,
                            DefaultDurationMin = 300,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 360,
                            MinDurationMin = 240,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 2,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 205,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 2,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 206,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 2,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 207,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 2,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 208,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 2,
                            StepMin = 60,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 209,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 2,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 210,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 2,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 211,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 2,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 212,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 2,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 301,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour and water only — no salt, no starter yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 3,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 302,
                            DefaultDurationMin = 20,
                            Description = "Cover and leave undisturbed. Enzymes hydrate flour and begin softening gluten.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Autolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 3,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 303,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate. Add starter and fold in fully.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt + starter",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 3,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 304,
                            DefaultDurationMin = 240,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 300,
                            MinDurationMin = 180,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 3,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 305,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 3,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 306,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 3,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 307,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 3,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 308,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 3,
                            StepMin = 30,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 309,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 3,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 310,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 3,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 311,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 3,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 312,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 3,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 401,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour, water, and starter — no salt yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water + starter",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 4,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 402,
                            DefaultDurationMin = 25,
                            Description = "Cover and leave undisturbed. Fermentation begins — watch for over-activity in warm kitchens.",
                            MaxDurationMin = 30,
                            MinDurationMin = 20,
                            Name = "Fermentolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 4,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 403,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate thoroughly.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 4,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 404,
                            DefaultDurationMin = 240,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 300,
                            MinDurationMin = 180,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 4,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 405,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 4,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 406,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 4,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 407,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 4,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 408,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 4,
                            StepMin = 60,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 409,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 4,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 410,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 4,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 411,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 4,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 412,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 4,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 501,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour and water only — no salt, no starter yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 5,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 502,
                            DefaultDurationMin = 10,
                            Description = "Cover and leave undisturbed. Enzymes hydrate flour and begin softening gluten.",
                            MaxDurationMin = 15,
                            MinDurationMin = 5,
                            Name = "Autolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 5,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 503,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate. Add starter and fold in fully.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt + starter",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 5,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 504,
                            DefaultDurationMin = 180,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 240,
                            MinDurationMin = 120,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 5,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 505,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 5,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 506,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 5,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 507,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 5,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 508,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 5,
                            StepMin = 30,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 509,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 5,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 510,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 5,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 511,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 5,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 512,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 5,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 601,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour, water, and starter — no salt yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water + starter",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 6,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 602,
                            DefaultDurationMin = 13,
                            Description = "Cover and leave undisturbed. Fermentation begins — watch for over-activity in warm kitchens.",
                            MaxDurationMin = 15,
                            MinDurationMin = 10,
                            Name = "Fermentolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 6,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 603,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate thoroughly.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 6,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 604,
                            DefaultDurationMin = 180,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 240,
                            MinDurationMin = 120,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 6,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 605,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 6,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 606,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 6,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 607,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 6,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 608,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 6,
                            StepMin = 60,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 609,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 6,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 610,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 6,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 611,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 6,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 612,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 6,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 701,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour and water only — no salt, no starter yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 7,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 702,
                            DefaultDurationMin = 15,
                            Description = "Cover and leave undisturbed. Enzymes hydrate flour and begin softening gluten.",
                            MaxDurationMin = 20,
                            MinDurationMin = 10,
                            Name = "Autolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 7,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 703,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate. Add starter and fold in fully.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt + starter",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 7,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 704,
                            DefaultDurationMin = 210,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 270,
                            MinDurationMin = 150,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 7,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 705,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 7,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 706,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 7,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 707,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 7,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 708,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 7,
                            StepMin = 30,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 709,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 7,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 710,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 7,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 711,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 7,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 712,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 7,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 801,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour, water, and starter — no salt yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water + starter",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 8,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 802,
                            DefaultDurationMin = 18,
                            Description = "Cover and leave undisturbed. Fermentation begins — watch for over-activity in warm kitchens.",
                            MaxDurationMin = 20,
                            MinDurationMin = 15,
                            Name = "Fermentolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 8,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 803,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate thoroughly.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 8,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 804,
                            DefaultDurationMin = 210,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 270,
                            MinDurationMin = 150,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 8,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 805,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 8,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 806,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 8,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 807,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 8,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 808,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 8,
                            StepMin = 60,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 809,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 8,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 810,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 8,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 811,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 8,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 812,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 8,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 901,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour and water only — no salt, no starter yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 9,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 902,
                            DefaultDurationMin = 37,
                            Description = "Cover and leave undisturbed. Enzymes hydrate flour and begin softening gluten.",
                            MaxDurationMin = 45,
                            MinDurationMin = 30,
                            Name = "Autolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 9,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 903,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate. Add starter and fold in fully.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt + starter",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 9,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 904,
                            DefaultDurationMin = 330,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 420,
                            MinDurationMin = 270,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 9,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 905,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 9,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 906,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 9,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 907,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 9,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 908,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 9,
                            StepMin = 30,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 909,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 9,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 910,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 9,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 911,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 9,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 912,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 9,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 1001,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour, water, and starter — no salt yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water + starter",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 10,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1002,
                            DefaultDurationMin = 40,
                            Description = "Cover and leave undisturbed. Fermentation begins — watch for over-activity in warm kitchens.",
                            MaxDurationMin = 45,
                            MinDurationMin = 35,
                            Name = "Fermentolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 10,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1003,
                            DefaultDurationMin = 5,
                            Description = "Dimple salt in and fold to incorporate thoroughly.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Add salt",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 10,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1004,
                            DefaultDurationMin = 330,
                            Description = "4–6 sets of stretch & folds every 30 min. Dough should grow 50–75%.",
                            MaxDurationMin = 420,
                            MinDurationMin = 270,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 10,
                            StepMin = 15,
                            TargetTempC = 23.0
                        },
                        new
                        {
                            Id = 1005,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape. Flour the bench lightly.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 10,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1006,
                            DefaultDurationMin = 30,
                            Description = "Leave uncovered on bench. Let gluten relax before final shape.",
                            MaxDurationMin = 45,
                            MinDurationMin = 20,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 10,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1007,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place seam-side up in floured banneton.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 10,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1008,
                            DefaultDurationMin = 960,
                            Description = "Retard in fridge overnight. Develops flavour and structure.",
                            MaxDurationMin = 1440,
                            MinDurationMin = 480,
                            Name = "Cold proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 10,
                            StepMin = 60,
                            TargetTempC = 4.0
                        },
                        new
                        {
                            Id = 1009,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 10,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 1010,
                            DefaultDurationMin = 20,
                            Description = "Steam inside Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 10,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 1011,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep caramelised crust. Internal temp should reach 96–98 °C.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 10,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 1012,
                            DefaultDurationMin = 120,
                            Description = "Crumb is still setting during cooling — do not cut early.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 10,
                            StepMin = 30
                        },
                        new
                        {
                            Id = 1101,
                            DefaultDurationMin = 5,
                            Description = "Combine flour, liquid, and binder (psyllium / xanthan / egg). No kneading.",
                            MaxDurationMin = 10,
                            MinDurationMin = 3,
                            Name = "Whisk batter",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 11,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1102,
                            DefaultDurationMin = 40,
                            Description = "Cover and rest. Starch absorbs liquid — reduces grittiness, improves crumb texture.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Soaker rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 11,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1103,
                            DefaultDurationMin = 5,
                            Description = "Fold in salt, any sweetener, fat, and starter if using sourdough.",
                            MaxDurationMin = 10,
                            MinDurationMin = 3,
                            Name = "Add remaining ingredients",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 11,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1104,
                            DefaultDurationMin = 90,
                            Description = "Gluten-free batter rises quickly and less dramatically. Watch for bubbles.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Ferment / proof",
                            Order = 4,
                            Phase = "Proof",
                            RecipeId = 11,
                            StepMin = 15,
                            TargetTempC = 25.0
                        },
                        new
                        {
                            Id = 1105,
                            DefaultDurationMin = 20,
                            Description = "Cover with foil or lid to trap steam and prevent early over-crust.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake in tin (covered)",
                            Order = 5,
                            Phase = "Bake",
                            RecipeId = 11,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 1106,
                            DefaultDurationMin = 30,
                            Description = "Until crust is set and internal temperature reaches 96–98 °C.",
                            MaxDurationMin = 40,
                            MinDurationMin = 25,
                            Name = "Bake uncovered",
                            Order = 6,
                            Phase = "Bake",
                            RecipeId = 11,
                            StepMin = 5,
                            TargetTempC = 200.0
                        },
                        new
                        {
                            Id = 1107,
                            DefaultDurationMin = 90,
                            Description = "Gluten-free crumb continues setting during cooling — do not cut early.",
                            MaxDurationMin = 120,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 7,
                            Phase = "Cool",
                            RecipeId = 11,
                            StepMin = 15
                        },
                        new
                        {
                            Id = 1201,
                            DefaultDurationMin = 5,
                            Description = "Combine flour, liquid, and binder (psyllium / xanthan / egg). No kneading.",
                            MaxDurationMin = 10,
                            MinDurationMin = 3,
                            Name = "Whisk batter",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 12,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1202,
                            DefaultDurationMin = 45,
                            Description = "Cover and rest. Starch absorbs liquid — reduces grittiness, improves crumb texture.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Soaker rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 12,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1203,
                            DefaultDurationMin = 5,
                            Description = "Fold in salt, any sweetener, fat, and starter if using sourdough.",
                            MaxDurationMin = 10,
                            MinDurationMin = 3,
                            Name = "Add remaining ingredients",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 12,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1204,
                            DefaultDurationMin = 90,
                            Description = "Gluten-free batter rises quickly and less dramatically. Watch for bubbles.",
                            MaxDurationMin = 180,
                            MinDurationMin = 60,
                            Name = "Ferment / proof",
                            Order = 4,
                            Phase = "Proof",
                            RecipeId = 12,
                            StepMin = 15,
                            TargetTempC = 25.0
                        },
                        new
                        {
                            Id = 1205,
                            DefaultDurationMin = 20,
                            Description = "Cover with foil or lid to trap steam and prevent early over-crust.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake in tin (covered)",
                            Order = 5,
                            Phase = "Bake",
                            RecipeId = 12,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 1206,
                            DefaultDurationMin = 30,
                            Description = "Until crust is set and internal temperature reaches 96–98 °C.",
                            MaxDurationMin = 40,
                            MinDurationMin = 25,
                            Name = "Bake uncovered",
                            Order = 6,
                            Phase = "Bake",
                            RecipeId = 12,
                            StepMin = 5,
                            TargetTempC = 200.0
                        },
                        new
                        {
                            Id = 1207,
                            DefaultDurationMin = 90,
                            Description = "Gluten-free crumb continues setting during cooling — do not cut early.",
                            MaxDurationMin = 120,
                            MinDurationMin = 60,
                            Name = "Cool on rack",
                            Order = 7,
                            Phase = "Cool",
                            RecipeId = 12,
                            StepMin = 15
                        },
                        new
                        {
                            Id = 1301,
                            DefaultDurationMin = 10,
                            Description = "Combine cake/pastry flour with warm water (38–40 °C). Add yeast and sugar. Mix until smooth.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Mix flour + water",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 13,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1302,
                            DefaultDurationMin = 60,
                            Description = "Cover and keep warm. Rise until doubled. Keep dough temperature 26–28 °C for even, predictable rise.",
                            MaxDurationMin = 90,
                            MinDurationMin = 45,
                            Name = "Bulk fermentation — rise until doubled",
                            Order = 2,
                            Phase = "Bulk",
                            RecipeId = 13,
                            StepMin = 15,
                            TargetTempC = 28.0
                        },
                        new
                        {
                            Id = 1303,
                            DefaultDurationMin = 10,
                            Description = "Punch down to degas. Divide into equal portions (50–80 g for buns). Cover resting pieces.",
                            MaxDurationMin = 15,
                            MinDurationMin = 5,
                            Name = "Knock back + portion",
                            Order = 3,
                            Phase = "Shape",
                            RecipeId = 13,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1304,
                            DefaultDurationMin = 15,
                            Description = "Roll each piece smooth or flatten and add filling. Place on parchment squares in the steamer.",
                            MaxDurationMin = 20,
                            MinDurationMin = 10,
                            Name = "Final shape",
                            Order = 4,
                            Phase = "Shape",
                            RecipeId = 13,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1305,
                            DefaultDurationMin = 20,
                            Description = "Proof in steamer with lid on but heat OFF. Buns should puff slightly and feel soft. Do not over-proof.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Final proof",
                            Order = 5,
                            Phase = "Proof",
                            RecipeId = 13,
                            StepMin = 5,
                            TargetTempC = 28.0
                        },
                        new
                        {
                            Id = 1306,
                            DefaultDurationMin = 15,
                            Description = "Bring water to a vigorous simmer, then steam. Line lid with a cloth to prevent drip marks.",
                            MaxDurationMin = 18,
                            MinDurationMin = 12,
                            Name = "Steam",
                            Order = 6,
                            Phase = "Bake",
                            RecipeId = 13,
                            StepMin = 2,
                            TargetTempC = 100.0
                        },
                        new
                        {
                            Id = 1307,
                            DefaultDurationMin = 3,
                            Description = "Tilt lid slightly for 2–3 min before removing entirely — prevents skin collapse from cold air shock.",
                            MaxDurationMin = 5,
                            MinDurationMin = 2,
                            Name = "Rest with lid ajar",
                            Order = 7,
                            Phase = "Bake",
                            RecipeId = 13,
                            StepMin = 1
                        },
                        new
                        {
                            Id = 1308,
                            DefaultDurationMin = 15,
                            Description = "Remove from steamer and cool briefly on a rack. Best eaten warm within the hour.",
                            MaxDurationMin = 20,
                            MinDurationMin = 10,
                            Name = "Cool on rack",
                            Order = 8,
                            Phase = "Cool",
                            RecipeId = 13,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1401,
                            DefaultDurationMin = 10,
                            Description = "Cook flour (6% of total) + liquid (5× flour weight) to ~65 °C, stirring constantly until a thick, glossy paste. Starch gelatinises — this is the key to the pillowy crumb.",
                            MaxDurationMin = 15,
                            MinDurationMin = 8,
                            Name = "Prepare Tangzhong",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 14,
                            StepMin = 2,
                            TargetTempC = 65.0
                        },
                        new
                        {
                            Id = 1402,
                            DefaultDurationMin = 30,
                            Description = "Spread on a plate or press cling film to the surface to prevent a skin. Must reach 25 °C before mixing — adding it hot will kill the yeast.",
                            MaxDurationMin = 60,
                            MinDurationMin = 20,
                            Name = "Cool Tangzhong",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 14,
                            StepMin = 10
                        },
                        new
                        {
                            Id = 1403,
                            DefaultDurationMin = 15,
                            Description = "Combine flour, milk, egg, sugar, salt, yeast, and cooled Tangzhong. Mix until shaggy, then knead to a smooth, non-sticky dough. No butter yet — gluten must develop first.",
                            MaxDurationMin = 20,
                            MinDurationMin = 10,
                            Name = "Mix dough (pre-butter)",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 14,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1404,
                            DefaultDurationMin = 15,
                            Description = "Add cold butter in small cubes while the dough is mixing. Dough will look broken — keep going. Stop when it passes the window-pane test: silky, elastic, and no longer sticky.",
                            MaxDurationMin = 25,
                            MinDurationMin = 10,
                            Name = "Add butter (window-pane)",
                            Order = 4,
                            Phase = "Mix",
                            RecipeId = 14,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1405,
                            DefaultDurationMin = 60,
                            Description = "Cover and keep warm at 26–28 °C. Enriched doughs rise faster than sourdough — watch for doubling in size, not just time elapsed.",
                            MaxDurationMin = 90,
                            MinDurationMin = 45,
                            Name = "Bulk fermentation (until doubled)",
                            Order = 5,
                            Phase = "Bulk",
                            RecipeId = 14,
                            StepMin = 15,
                            TargetTempC = 27.0
                        },
                        new
                        {
                            Id = 1406,
                            DefaultDurationMin = 10,
                            Description = "Scale portions equally. Gently degas and pre-shape into rounds. Cover and rest 5 min.",
                            MaxDurationMin = 15,
                            MinDurationMin = 5,
                            Name = "Divide + pre-shape",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 14,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1407,
                            DefaultDurationMin = 15,
                            Description = "Rest covered — gluten tightens after pre-shape. This relaxation makes the final roll-and-fold possible without tearing.",
                            MaxDurationMin = 20,
                            MinDurationMin = 10,
                            Name = "Bench rest",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 14,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1408,
                            DefaultDurationMin = 15,
                            Description = "Roll each piece flat, fold the sides in, roll up tightly, and place seam-down in the greased tin. For Pullman, fill the tin 70% and add the lid.",
                            MaxDurationMin = 20,
                            MinDurationMin = 10,
                            Name = "Final shape + tin",
                            Order = 8,
                            Phase = "Shape",
                            RecipeId = 14,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1409,
                            DefaultDurationMin = 60,
                            Description = "Proof at 28–30 °C until dough is 80–90% of tin height (open tin: dome 2–3 cm above rim). Over-proofing collapses the crumb after baking.",
                            MaxDurationMin = 90,
                            MinDurationMin = 45,
                            Name = "Final proof (80–90% tin height)",
                            Order = 9,
                            Phase = "Proof",
                            RecipeId = 14,
                            StepMin = 15,
                            TargetTempC = 29.0
                        },
                        new
                        {
                            Id = 1410,
                            DefaultDurationMin = 30,
                            Description = "Pullman: bake with lid closed for 25 min, then remove lid for 5 min for colour. Open tin: bake uncovered 30 min. Internal temp target ~88 °C.",
                            MaxDurationMin = 35,
                            MinDurationMin = 25,
                            Name = "Bake (Pullman lidded / open tin)",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 14,
                            StepMin = 5,
                            TargetTempC = 190.0
                        },
                        new
                        {
                            Id = 1411,
                            DefaultDurationMin = 60,
                            Description = "Cool in tin 5 min, then unmould onto a rack. Slice only when fully cool — the crumb is still setting as it cools and cuts gummy if sliced hot.",
                            MaxDurationMin = 120,
                            MinDurationMin = 30,
                            Name = "Cool on rack",
                            Order = 11,
                            Phase = "Cool",
                            RecipeId = 14,
                            StepMin = 15
                        },
                        new
                        {
                            Id = 1501,
                            DefaultDurationMin = 5,
                            Description = "Rough mix of flour and water only — no yeast, no salt yet.",
                            MaxDurationMin = 15,
                            MinDurationMin = 3,
                            Name = "Mix flour + water",
                            Order = 1,
                            Phase = "Mix",
                            RecipeId = 15,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1502,
                            DefaultDurationMin = 20,
                            Description = "Cover and rest. Enzymes hydrate the flour and begin softening gluten.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Autolyse rest",
                            Order = 2,
                            Phase = "Rest",
                            RecipeId = 15,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1503,
                            DefaultDurationMin = 5,
                            Description = "Add instant yeast (0.5–2% of flour weight) and salt. Fold to incorporate.",
                            MaxDurationMin = 10,
                            MinDurationMin = 3,
                            Name = "Add yeast + salt",
                            Order = 3,
                            Phase = "Mix",
                            RecipeId = 15,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1504,
                            DefaultDurationMin = 90,
                            Description = "3–4 sets of stretch & folds every 20 min. Dough should roughly double in volume.",
                            MaxDurationMin = 120,
                            MinDurationMin = 60,
                            Name = "Bulk fermentation",
                            Order = 4,
                            Phase = "Bulk",
                            RecipeId = 15,
                            StepMin = 15,
                            TargetTempC = 25.0
                        },
                        new
                        {
                            Id = 1505,
                            DefaultDurationMin = 10,
                            Description = "Gentle pre-shape on a lightly floured bench.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Pre-shape",
                            Order = 5,
                            Phase = "Shape",
                            RecipeId = 15,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1506,
                            DefaultDurationMin = 20,
                            Description = "Leave uncovered. Gluten relaxes before final shaping.",
                            MaxDurationMin = 30,
                            MinDurationMin = 15,
                            Name = "Bench rest",
                            Order = 6,
                            Phase = "Shape",
                            RecipeId = 15,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1507,
                            DefaultDurationMin = 10,
                            Description = "Build surface tension. Place in floured banneton or greased tin.",
                            MaxDurationMin = 20,
                            MinDurationMin = 5,
                            Name = "Final shape",
                            Order = 7,
                            Phase = "Shape",
                            RecipeId = 15,
                            StepMin = 5
                        },
                        new
                        {
                            Id = 1508,
                            DefaultDurationMin = 60,
                            Description = "Proof until visibly puffed and the dough springs back slowly when poked.",
                            MaxDurationMin = 90,
                            MinDurationMin = 45,
                            Name = "Room-temperature proof",
                            Order = 8,
                            Phase = "Proof",
                            RecipeId = 15,
                            StepMin = 15,
                            TargetTempC = 25.0
                        },
                        new
                        {
                            Id = 1509,
                            DefaultDurationMin = 45,
                            Description = "Cast iron must be screaming hot before the loaf goes in.",
                            MaxDurationMin = 60,
                            MinDurationMin = 30,
                            Name = "Preheat + Dutch oven",
                            Order = 9,
                            Phase = "Bake",
                            RecipeId = 15,
                            StepMin = 15,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 1510,
                            DefaultDurationMin = 20,
                            Description = "Steam trapped in the Dutch oven drives oven spring and crust formation.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake covered",
                            Order = 10,
                            Phase = "Bake",
                            RecipeId = 15,
                            StepMin = 5,
                            TargetTempC = 250.0
                        },
                        new
                        {
                            Id = 1511,
                            DefaultDurationMin = 20,
                            Description = "Achieve deep golden crust. Internal temperature should reach 96–98 °C.",
                            MaxDurationMin = 25,
                            MinDurationMin = 15,
                            Name = "Bake uncovered",
                            Order = 11,
                            Phase = "Bake",
                            RecipeId = 15,
                            StepMin = 5,
                            TargetTempC = 220.0
                        },
                        new
                        {
                            Id = 1512,
                            DefaultDurationMin = 60,
                            Description = "Do not cut early — the crumb is still setting during cooling.",
                            MaxDurationMin = 90,
                            MinDurationMin = 45,
                            Name = "Cool on rack",
                            Order = 12,
                            Phase = "Cool",
                            RecipeId = 15,
                            StepMin = 15
                        });
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Starter", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("FlourBlend")
                        .HasColumnType("TEXT");

                    b.Property<double>("HydrationPct")
                        .HasColumnType("REAL");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("Notes")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("Starters");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.StarterFeedLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<double?>("AmbientTempC")
                        .HasColumnType("REAL");

                    b.Property<DateTimeOffset>("FedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("FeedRatio")
                        .HasColumnType("TEXT");

                    b.Property<bool?>("FloatTestPassed")
                        .HasColumnType("INTEGER");

                    b.Property<double>("FlourGrams")
                        .HasColumnType("REAL");

                    b.Property<double?>("PeakHours")
                        .HasColumnType("REAL");

                    b.Property<double>("PrevStarterGrams")
                        .HasColumnType("REAL");

                    b.Property<int>("StarterId")
                        .HasColumnType("INTEGER");

                    b.Property<double>("WaterGrams")
                        .HasColumnType("REAL");

                    b.HasKey("Id");

                    b.HasIndex("StarterId");

                    b.ToTable("StarterFeedLogs");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Bake", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.Recipe", "Recipe")
                        .WithMany("Bakes")
                        .HasForeignKey("RecipeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BreadMaking.App.Server.Data.StarterFeedLog", "StarterFeed")
                        .WithMany()
                        .HasForeignKey("StarterFeedLogId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Recipe");

                    b.Navigation("StarterFeed");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.BakeOutcome", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.Bake", "Bake")
                        .WithOne("Outcome")
                        .HasForeignKey("BreadMaking.App.Server.Data.BakeOutcome", "BakeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Bake");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.BakeStepLog", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.Bake", "Bake")
                        .WithMany("StepLogs")
                        .HasForeignKey("BakeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BreadMaking.App.Server.Data.RecipeStep", "RecipeStep")
                        .WithMany("StepLogs")
                        .HasForeignKey("RecipeStepId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Bake");

                    b.Navigation("RecipeStep");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.FormulaIngredient", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.BreadFormula", "Formula")
                        .WithMany("Ingredients")
                        .HasForeignKey("FormulaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Formula");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Measurement", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.BakeStepLog", "BakeStepLog")
                        .WithMany("Measurements")
                        .HasForeignKey("BakeStepLogId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BreadMaking.App.Server.Data.MeasurementType", "MeasurementType")
                        .WithMany("Measurements")
                        .HasForeignKey("MeasurementTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BakeStepLog");

                    b.Navigation("MeasurementType");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Recipe", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.GrainProfileEntity", "GrainProfile")
                        .WithMany("Recipes")
                        .HasForeignKey("GrainProfileId");

                    b.Navigation("GrainProfile");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.RecipeFormula", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.Recipe", "Recipe")
                        .WithOne("Formula")
                        .HasForeignKey("BreadMaking.App.Server.Data.RecipeFormula", "RecipeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Recipe");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.RecipeStep", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.Recipe", "Recipe")
                        .WithMany("Steps")
                        .HasForeignKey("RecipeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Recipe");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.StarterFeedLog", b =>
                {
                    b.HasOne("BreadMaking.App.Server.Data.Starter", "Starter")
                        .WithMany("Feeds")
                        .HasForeignKey("StarterId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Starter");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Bake", b =>
                {
                    b.Navigation("Outcome");

                    b.Navigation("StepLogs");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.BakeStepLog", b =>
                {
                    b.Navigation("Measurements");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.BreadFormula", b =>
                {
                    b.Navigation("Ingredients");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.GrainProfileEntity", b =>
                {
                    b.Navigation("Recipes");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.MeasurementType", b =>
                {
                    b.Navigation("Measurements");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Recipe", b =>
                {
                    b.Navigation("Bakes");

                    b.Navigation("Formula");

                    b.Navigation("Steps");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.RecipeStep", b =>
                {
                    b.Navigation("StepLogs");
                });

            modelBuilder.Entity("BreadMaking.App.Server.Data.Starter", b =>
                {
                    b.Navigation("Feeds");
                });
#pragma warning restore 612, 618
        }
    }
}
